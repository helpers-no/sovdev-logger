...
- [`INVESTIGATE-repo-and-package-ownership.md`](INVESTIGATE-repo-and-package-ownership.md) — **Part 1 shipped 2026-07-13**: `helpers-no/sovdev-logger` was a GitHub fork of the now-inactive `norwegianredcross/sovdev-logger`; maintainer skipped the GitHub Support route and had it copied outright — renamed the old fork to `sovdev-logger-old-fork` (archived, issue #23 preserved there), created a genuinely fresh non-fork repo at the same name, mirror-pushed full history, and moved GitHub Pages/the custom domain over with effectively no downtime. **Part 2 still open**: the npm package `@terchris/sovdev-logger` remains on the maintainer's personal npm account, not an org — needs a scope name picked ([Q3]) before a `PLAN-npm-package-rename.md` can be drafted.
