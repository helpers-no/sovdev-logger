...
- [`INVESTIGATE-repo-and-package-ownership.md`](INVESTIGATE-repo-and-package-ownership.md) — `helpers-no/sovdev-logger` is technically a GitHub fork of the now-inactive `norwegianredcross/sovdev-logger` (development moved here for speed, but the fork direction was never flipped), and the npm package `@terchris/sovdev-logger` is owned by the maintainer's personal npm account, not an org. Both are fixable — GitHub fork detachment via a support ticket (no self-service/API route), npm via publishing a new org-scoped name and deprecating the old one (no in-place rename) — and both are cheaper to fix now than later: the maintainer manages both GitHub orgs and the only known npm consumer (`terchris/ollacrm`), so there's no third-party coordination required yet. 5 open questions need maintainer answers — see the doc.
