...
**Last triaged:** 2026-07-13 — maintainer asked to check whether OTel had actually been upgraded yet (it hadn't — still exactly where `INVESTIGATE-otel-dependency-upgrade.md` left it), then to investigate updating *every* software component in the repo: new [`INVESTIGATE-dependency-upgrade-sweep.md`](INVESTIGATE-dependency-upgrade-sweep.md) checked every `package.json`/`requirements.txt`/workflow file directly and found real drift everywhere except Python (already current by design, unpinned `>=` constraints) and the DevContainer image (already tracks `:latest`, externally managed) — TypeScript dev tooling, Docusaurus, and GitHub Actions are all 1-3 major/minor versions behind, on top of the already-known OTel/security gap.
>
...
- [`INVESTIGATE-otel-dependency-upgrade.md`](INVESTIGATE-otel-dependency-upgrade.md) — 4 Dependabot alerts (2 high, 2 moderate) against `@opentelemetry/sdk-node` (`^0.55.0`, ~165 minor versions behind), `@opentelemetry/auto-instrumentations-node` (`^0.51.0`, ~27 behind), and `@opentelemetry/core` (a full major version behind, `1.x` → `2.x`); `uuid` is also imported directly in source but was never declared as a direct dependency. **Re-checked 2026-07-13, worse than documented**: `gh api .../dependabot/alerts` on upstream and a local `npm audit` both show the real count is 49 vulnerabilities (45 moderate, 3 high, **1 critical** — arbitrary code execution in `protobufjs`, a transitive OTel dependency never counted here) — the doc's "4 alerts" only covered direct OTel/uuid entries, not the transitive `protobufjs`/`@grpc/grpc-js`/`js-yaml` chain underneath them. Still needs a deliberate bump strategy (OTel's `0.x` packages can break on a minor bump per semver), but this should be re-scoped with the real number before drafting a `PLAN-*.md`.
